...
def start(left, right, key):
    rr = round(right, key)
    textLeft = left^rr
    return textLeft+right
...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    if mode == MODE_ENCRYPT:
        key1 = schedule_keys(key)[0]
        key2 = schedule_keys(key)[1]
    elif mode == MODE_DECRYPT:
        key1 = schedule_keys(key)[1]
        key2 = schedule_keys(key)[0]

    t = start(text[0:4], text[4:8], key1)
    result = start(t[4:8], t[0:4], key2)
